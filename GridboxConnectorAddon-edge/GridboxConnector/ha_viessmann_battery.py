...
"""
    ...
    def __init__(self, mqtt_settings, device_info, name, id, prefix="", unit_of_power="W", state_class=None):
        ...
        self.battery_sensor_power = SensorInfo(
            name=f"Battery {name} Power", device_class="battery", unique_id=f"gridbox_battery_power_{name}"+prefix, device=device_info, unit_of_measurement=unit_of_power, state_class=state_class, value_template=None if state_class is None else "{{ value_json.state }}", last_reset_value_template=None if state_class is None else "{{ value_json.last_reset }}")
        ...
        self.battery_sensor_remaining_charge = SensorInfo(
            name=f"Battery {name} Remaining Charge", device_class="energy", unique_id=f"gridbox_remaining_charge_{name}"+prefix, device=device_info, unit_of_measurement="Wh", state_class=state_class, value_template=None if state_class is None else "{{ value_json.state }}", last_reset_value_template=None if state_class is None else "{{ value_json.last_reset }}")
    ...
    def set_states(self, level, capacity, power, remaining_charge, charge=None, discharge=None):
