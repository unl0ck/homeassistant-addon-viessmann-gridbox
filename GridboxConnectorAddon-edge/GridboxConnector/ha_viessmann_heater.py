...
class HAViessmannHeater:
    def __init__(self, mqtt_settings, device_info, name, id):
        ...
        self.heater_sensor_power = SensorInfo(name=f"Heater {name} Power", device_class="energy", unique_id=f"gridbox_heater_power_{name}", device=device_info, unit_of_measurement="W")
        self.heater_power_settings = Settings(mqtt=mqtt_settings, entity=self.heater_sensor_power)
        ...
        self.heater_temperature_settings = Settings(mqtt=mqtt_settings, entity=self.heater_sensor_temperature)
