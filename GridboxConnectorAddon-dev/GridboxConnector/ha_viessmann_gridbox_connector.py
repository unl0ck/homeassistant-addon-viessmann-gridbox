...
class HAViessmannGridboxConnector:
    ...
    grid_sensor: Sensor
    feed_in_sensor: Sensor
    supply_sensor: Sensor
    ...
    def __init__(
        self,
        mqtt_settings,
        device_name="Viessmann Gridbox",
        device_identifiers="viessmann_gridbox",
        device_manufacturer="Viessmann",
        device_model="Vitocharge 2.0",
        prefix="",
        logger=logging.getLogger(__name__),
        device_class_of_power="power",
        unit_of_power="W",
        state_class=None,
    ):
        ...
        production_sensor_info = SensorInfo(
            name="Production",
            device_class=device_class_of_power,
            unique_id="gridbox_production" + prefix,
        ...
        if state_class is None:
            grid_sensor_info = SensorInfo(
                name="Grid",
                device_class=device_class_of_power,
                unique_id="gridbox_grid" + prefix,
                device=self.device_info,
                unit_of_measurement=unit_of_power,
                state_class=state_class,
                value_template=None
            ...
            feed_in_sensor_info = SensorInfo(
                name="FeedIn",
                device_class=device_class_of_power,
                unique_id="gridbox_feed_in" + prefix,
                device=self.device_info,
                unit_of_measurement=unit_of_power,
                state_class=state_class,
                value_template=None
            ...
            feed_in_settings = Settings(mqtt=mqtt_settings, entity=feed_in_sensor_info)

            supply_sensor_info = SensorInfo(
                name="Supply",
                device_class=device_class_of_power,
                unique_id="gridbox_supply" + prefix,
                device=self.device_info,
                unit_of_measurement=unit_of_power,
                state_class=state_class,
                value_template=None
            ...
            supply_settings = Settings(mqtt=mqtt_settings, entity=supply_sensor_info)

        photovoltaic_sensor_info = SensorInfo(
            name="Photovoltaic",
            device_class=device_class_of_power,
            unique_id="gridbox_photovoltaic" + prefix,
            device=self.device_info,
            unit_of_measurement=unit_of_power,
            state_class=state_class,
            value_template=None if state_class is None else "{{ value_json.state }}",
        ...
        consumption_household_sensor_info = SensorInfo(
            name="Consumption",
            device_class=device_class_of_power,
            unique_id="gridbox_consumption_household" + prefix,
            device=self.device_info,
            unit_of_measurement=unit_of_power,
            state_class=state_class,
            value_template=None if state_class is None else "{{ value_json.state }}",
        ...
        total_consumption_household_sensor_info = SensorInfo(
            ...
            device_class=device_class_of_power,
            unique_id="total_consumption_household" + prefix,
            device=self.device_info,
            unit_of_measurement=unit_of_power,
            state_class=state_class,
            value_template=None if state_class is None else "{{ value_json.state }}",
        ...
        direct_consumption_household_sensor_info = SensorInfo(
            ...
            device_class=device_class_of_power,
            unique_id="gridbox_direct_consumption_household" + prefix,
            device=self.device_info,
            unit_of_measurement=unit_of_power,
            state_class=state_class,
            value_template=None if state_class is None else "{{ value_json.state }}",
        ...
        direct_consumption_heatpump_sensor_info = SensorInfo(
            ...
            device_class=device_class_of_power,
            unique_id="gridbox_direct_consumption_heatpump" + prefix,
            device=self.device_info,
            unit_of_measurement=unit_of_power,
            state_class=state_class,
            value_template=None if state_class is None else "{{ value_json.state }}",
        ...
        direct_consumption_ev_sensor_info = SensorInfo(
            name="DirectConsumptionEV",
            device_class=device_class_of_power,
            unique_id="gridbox_direct_consumption_ev" + prefix,
            device=self.device_info,
            unit_of_measurement=unit_of_power,
            state_class=state_class,
            value_template=None if state_class is None else "{{ value_json.state }}",
        ...
        direct_consumption_ev_settings = Settings(
            mqtt=mqtt_settings, entity=direct_consumption_ev_sensor_info
        )

        direct_consumption_rate_sensor_info = SensorInfo(
            name="DirectConsumptionRate",
            device_class="power_factor",
            unique_id="gridbox_direct_consumption_rate" + prefix,
            device=self.device_info,
            unit_of_measurement="%",
        )
        ...
        # Self Consumption
        self_supply_sensor_info = SensorInfo(
            name="SelfSupply",
            device_class=device_class_of_power,
            unique_id="gridbox_self_supply" + prefix,
            device=self.device_info,
            unit_of_measurement=unit_of_power,
            state_class=state_class,
            value_template=None if state_class is None else "{{ value_json.state }}",
        ...
        self_consumption_rate_sensor_info = SensorInfo(
            ...
            device_class="power_factor",
            unique_id="gridbox_self_consumption_rate" + prefix,
            device=self.device_info,
            unit_of_measurement="%",
        )
        ...
        self_sufficiency_rate_sensor_info = SensorInfo(
            ...
            device_class="power_factor",
            unique_id="gridbox_self_sufficiency_rate" + prefix,
            device=self.device_info,
            unit_of_measurement="%",
        )
        ...
        if state_class is None:
            ...
            self.feed_in_sensor = Sensor(feed_in_settings)
            self.supply_sensor = Sensor(supply_settings)
        ...
        self.heater_sensor = HAViessmannHeater(
            mqtt_settings, self.device_info, "", "", prefix
        )

        # EV
        self.ev_sum = HAViessmannEVChargingStation(
            mqtt_settings, self.device_info, "sum", "", prefix
        )
        ...
        self.direct_consumption_heatpump_sensor = Sensor(
    ...
    def update_sensors(self, measurement: dict, last_reset: str = None):
        if "production" in measurement:
            self.production_sensor.set_state(
                measurement.get("production", ""), last_reset=last_reset
            )
        else:
            self.logger.warning("No production data received")
        if "grid" in measurement:
            if last_reset is None:
                self.grid_sensor.set_state(
                    measurement.get("grid", ""), last_reset=last_reset
                )
            else:
                grid_object = measurement.get("grid", {})
                self.feed_in_sensor.set_state(
                    grid_object.get("feedIn", ""), last_reset=last_reset
                )
                self.supply_sensor.set_state(
                    grid_object.get("supply", ""), last_reset=last_reset
                )
        else:
            self.logger.warning("No grid data received")
        if "photovoltaic" in measurement:
            self.photovoltaic_sensor.set_state(
                measurement.get("photovoltaic", ""), last_reset=last_reset
            )
        else:
        ...
        if "directConsumptionEV" in measurement:
            self.direct_consumption_ev_sensor.set_state(
                float(measurement.get("directConsumptionEV", "0")),
                last_reset=last_reset,
            )
        ...
        if "evChargingStations" in measurement:
            ...
            for index, ev_charging_station in enumerate(ev_charging_stations):
                ...
                power = float(ev_charging_station.get("power", "0"))
                state_of_charge = (
                    float(ev_charging_station.get("stateOfCharge", "0")) * 100
                )
                current_l1 = float(ev_charging_station.get("currentL1", "0"))
                current_l2 = float(ev_charging_station.get("currentL2", "0"))
                current_l3 = float(ev_charging_station.get("currentL3", "0"))
